    ...
    aclOpExecutor **executor)
{
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttr(*executor, static_cast<void*>(&blockSize), sizeof(int64_t), 0));
    if (mode) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttr(*executor, static_cast<void*>(mode), strlen(mode) + 1, 1));
    } else {
        static char *modeDef = "DCR";
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttr(*executor, static_cast<void*>(modeDef), strlen(modeDef) + 1, 1));
    }
    if (dataFormat) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttr(*executor, static_cast<void*>(dataFormat), strlen(dataFormat) + 1, 2));
    } else {
        static char *dataFormatDef = "NHWC";
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttr(*executor, static_cast<void*>(dataFormatDef), strlen(dataFormatDef) + 1, 2));
    }

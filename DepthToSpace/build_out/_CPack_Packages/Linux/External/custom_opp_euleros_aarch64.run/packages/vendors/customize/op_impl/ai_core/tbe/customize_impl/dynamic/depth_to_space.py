...
def _build_args(x_in__, y_out_, block_size, mode, data_format):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
    ...
    if data_format != None:
        ...
        attr["dtype"] = "str"
        attr["value"] = data_format
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("DepthToSpace", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def depth_to_space(x_in__, y_out_, block_size, mode="DCR", data_format="NHWC", kernel_name="depth_to_space", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, block_size, mode, data_format)
    ...
    origin_func_name = "depth_to_space"
    ascendc_src_dir_ex = "depth_to_space"
    ascendc_src_dir = "depth_to_space"
    ascendc_src_file = "depth_to_space.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DepthToSpace. kernel name is " + kernel_name)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [y_out_], param_type_dynamic = False,\
        mc2_ctx = None, param_type_list = ['required', 'required'])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, y_out_, block_size, mode="DCR", data_format="NHWC", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, block_size, mode, data_format)
    result = check_op_cap("op_select_format", "DepthToSpace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, block_size, mode="DCR", data_format="NHWC", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, block_size, mode, data_format)
    result = check_op_cap("get_op_specific_info", "DepthToSpace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
